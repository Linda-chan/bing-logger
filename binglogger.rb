...
def get_page_by_url(target_url, ua, error_message, redirect_limit = 10)
  ...
  # При кажом повторном вызове функции, параметр уменьшается на один.
  # Если тут ноль, значит, хватит крутиться. Максимальное количество 
  # редиректов всегда на один меньше первоначального значения 
  # параметра. 
...
def parse_html_ang_get_picture_url(page_source)
  ...
  # получит знаение только если регэксп сработал.
  txt = ""
...
def get_new_picture_urls(picture_url)
  ...
  # с другими известными рсзрешениями. Сортируем по мере снижения 
