...
#====================================================================
# AJPapps - Bing logger Ruby ver.
# 
# Линда Кайе 2013-2018. Посвящается Ариэль
#====================================================================
# 
# Этот скрипт ни чего особого не делает. Он просто скачивает обойку 
# с главной страницы Bing и сохраняет в указанную папку с файлом 
# описания. Сохраняется только текущая обойки, другие, которые были 
# ранее, не трогаются. Я написала его, когда разбирала кэш Бинга. 
# Подумала, а почему бы и нет, и переделала скрипт 
# AJPapps - Nuerburgring logger ^^
# 
...
# 
# Так же имеется Ruby версия, которая работает везде, где есть 
# интерпретатор данного языка. Я тестировала в версиях 2.0.0 и выше. 
# Так же она требует гем unicode, установить который можно следующей 
# командой:
# 
# > gem install unicode
# 
# Так же при запуске Ruby версии под Windows может потребоваться 
# файл curl-ca-bundle.crt, содержащий в себе сертификаты доверенных 
# корневых центров сертификации, необходимых для протокола HTTPS. 
# Этот файл должен находится в одном каталоге со скриптом, и взять 
# его можно вместе с программой cURL вот по этому адресу:
# 
# https://curl.haxx.se
# 
# В крайнем случае можно заблокировать проверку сертификатов вообще 
# (не рекомендуется), разкомментировав данную строку в скрипте:
# 
...
# в первом примере и на Linux путь в третьем примере. Так же 
# я рекомендую указывать каталог точно во избежание несчастных 
# случаев. 
# 
...
# 
# В случае Ruby версии необходимо перенаправить вывод на NUL или 
...
# 
# • 6.06.2014
#   [+] Текстовые файлы теперь сохраняются с расширением в нижнем 
#       регистре. Для гармонии.
# 
# • 13.10.2014
...
# 
# • 2.12.2014
#   [-] На сайте изменили код страницы, и описание картинки 
#       перестало определяться.
#   [+] Настройки прокси вынесены в константу.
# 
# • 3.09.2015
#   [+] Скрипт пытается получить картинку максимального размера, 
#       а не ту, которую присылает Бинг.
...
#   [-] Если в будущем скрипт снова перестанет определять адрес 
#       или описание картинки, об этом будет сообщаться.
# 
# • 30.06.2016
#   [-] Перестал определяться адрес картинки.
# 
...
#   [-] HTR версия не учитывала тот факт, что Bing присылает 
#       картинку в UTF-8.
...
#   [+] Код WHT версии немного синхронизирован с WHT версией 
#       Download mota.ru Image для обхода возможных будущих проблем 
#       определения кодировки HTML5 файлов в WinHTTP (если Bing 
#       вообще перейдёт на HTML5).
#   
# • 15.01.2018
